...
### Фаза 3: Enum, Env, Вложенные секции

- [x] **Enum флаги** — `type = "enum", values = ["v1", "v2"], default = "v1"` с типобезопасными геттерами
- [x] **Env var override** — `--with-env-override` + `--env-var-prefix` через koanf env provider
- [x] **Вложенные секции** — `[oauth.github]` → `OauthGithub` struct с рекурсивной генерацией
- [x] **Локальные значения флагов** — секция `[flags]` в `config_local.toml` для override дефолтов
- [x] **Удалён union mode** — оставлен только intersect, для объединения используется Merge
...
- [x] **Флаги с enum** — `type = "enum", values = ["v1", "v2"], default = "v1"`
...
- [x] **Env var override** — `SERVER_HOST=override` через koanf env provider
- [ ] **Validation rules** — `# validate: min=1,max=65535` в комментариях TOML
- [x] **Вложенные секции** — `[server.tls]` → `ServerTls` struct
